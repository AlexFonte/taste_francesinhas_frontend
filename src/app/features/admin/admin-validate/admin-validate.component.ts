...
import {FrancesinhaInfoForm, RestaurantInfoForm} from './admin-validate.types';
...
export class AdminValidateComponent {
	...
	approve(): void {
		...
		this.openConfirm({
			...
			if (ok) this.changeStatus('ACCEPTED');
		});
	}
	...
	reject(): void {
		...
		this.openConfirm({
			...
			if (ok) this.changeStatus('REJECTED');
		});
	}
	...
	private load(id: number): void {
		...
		// La francesinha pendiente y la review  llegan juntas en un solo DTO,
		// asi que con una sola llamada tenemos todo lo que necesita la pantalla.
		this.adminService.getPendingById(id).subscribe({
			next: ({francesinha, review}) => {
				this.francesinha.set(francesinha);
				this.restaurantForm.patchValue({
					search: `${francesinha.restaurant.name} · ${francesinha.restaurant.city}`,
					name: francesinha.restaurant.name,
					city: francesinha.restaurant.city,
					address: francesinha.restaurant.address ?? '',
					phone: francesinha.restaurant.phone ?? '',
				});
				this.francesinhaForm.patchValue({
					name: francesinha.name,
					type: francesinha.type,
				...
				if (review) {
					...
					this.reviewForm.patchValue({
						scoreFlavor: review.scoreFlavor,
						scoreSauce: review.scoreSauce,
						scoreBread: review.scoreBread,
						scorePresentation: review.scorePresentation,
